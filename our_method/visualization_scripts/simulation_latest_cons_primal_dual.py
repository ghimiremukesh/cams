...
chkpnts = [f'../logs_cons_dual/t_{i}/checkpoint_0/checkpoint' for i in range(1, 11)]
...
primal_chkpnts = [f'../logs_cons/t_{i}/checkpoint_0/checkpoint' for i in range(1, 11)]
...
def dual_value_final(params, x, p_hat, t):
    ...
    # l3 = 1 - l1 - l2 + EPSILON
    # p_hat_3 = (p_hat - l1 * p_hat_1 - l2 * p_hat_2)/l3
    # p_hat_3 = p_hat_3.clip(-p_hat_bound, p_hat_bound) should not clip
    ...
    final_pen_1 = final_feasibility(x_next_1)
    final_pen_2 = final_feasibility(x_next_2)
    final_pen_3 = final_feasibility(x_next_3)

    objective = (l1 * val_1 + l2 * val_2 + l3 * val_3 + p_hat_penalty + lam_penalty
                 - 1000 * relu(-final_pen_1) - 1000 * relu(-final_pen_2) - 1000 * relu(-final_pen_3)) # + reg1 - reg2
...
def dual_value_inter(params, x, p_hat, t):
    ...
    from_brt_2, _ = brt_value(t - dt, x_next_2)
    from_brt_3, _ = brt_value(t - dt, x_next_3)

    objective = (l1 * val_1 + l2 * val_2 + l3 * val_3 + p_hat_penalty + lam_penalty - 1000 * relu(-from_brt_1) -
                 1000 * relu(-from_brt_2) - 1000 * relu(-from_brt_3))#+ reg1 - reg2
...
if __name__ == '__main__':
    ...
    for run in range(len(x1s)):  # number of simulations to run
        ...
        while t >= dt:
            ...
            # p_hat_3 = (p_hat_t - l1 * p_hat_1 - l2 * p_hat_2) / l3
            # p_hat_3 = p_hat_3.clip(-p_hat_bound, p_hat_bound)
