...
model_dir = os.path.join(current_dir, f'logs_uncons_more_data/t_{t_step - 1}/checkpoint_0/checkpoint')
config = ModelConfig
model = PICNN(config)

if t != dt:
    load_dir = f'logs_uncons_more_data/t_{t_step - 1}/checkpoint_0/checkpoint'
...
def solve_and_collect(pos_bound, num_points):
    ...
    params_u = jax.random.uniform(key5, (num_points, 4), minval=0, maxval=1)
    params_up = jnp.repeat(jnp.array([[EPSILON, 1-EPSILON]]), repeats=num_points, axis=0)
    params_d = jax.random.uniform(key6, (num_points, 4), minval=0, maxval=1)
