...
import time
from jax import jit
from utils_jax import final_cost_function, normalize_to_max_1d, normalize_to_max_final, final_cost_function_dual
...
if t != dt:
    load_dir = f'logs/dual_value_more_data/t_{t_step - 1}/checkpoint_0/checkpoint'
...
@jit
def dsgda_obj_inter(params, x, p_hat):
    r1 = 5
    r2 = 1
    v1 = params[:2]
    v2 = params[2:4]
    ...
    reg_1 = (r1 / 2) * jnp.sum((p2_params - z) ** 2)
    reg_2 = (r2 / 2) * jnp.sum((p1_params - v) ** 2)


    lam_penalty = 8 * relu(-l3)  # if lambda_3 is negative penalty is high
    p_hat_penalty = 8 * relu(cons_phat)
    objective = l1 * val_1 + l2 * val_2 + l3 * val_3 + p_hat_penalty + lam_penalty #+ reg_1 - reg_2 
...
@jit
def dsgda_obj_final(params, x, p_hat):
    r1 = 5
    r2 = 1
    v1 = params[:2]
    v2 = params[2:4]
    ...
    # cond is not good. need to penalize
    # l3_true = lambda: 1 - l1 - l2
    # l3_false = lambda: 0.
    ...
    lam_penalty = 8 * relu(-l3)  # if lambda_3 is negative penalty is high
    p_hat_penalty = 8 * relu(cons_phat)
    objective = l1 * val_1 + l2 * val_2 + l3 * val_3 + p_hat_penalty + lam_penalty #+ reg_1 - reg_2   # add regularization if necessary
