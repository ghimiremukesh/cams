...
import time
from jax import jit
from utils_jax import final_cost_function_3d, normalize_to_max_3d, normalize_to_max_final_3d

from utils_jax import x_next_3d, running_cost_3d
from functools import partial
from flax_picnn_3d import PICNN
from flax_picnn_3d import ModelConfig
...
model_dir = os.path.join(current_dir, f'logs_3d_case/t_{t_step - 1}/checkpoint_0/checkpoint')
config = ModelConfig
model = PICNN(config)

if t != dt:
    load_dir = f'logs_3d_case/t_{t_step - 1}/checkpoint_0/checkpoint'
...
def gradient_step(params, x, p, loss_fn):
    ...
    new_p2_params = (p2_params + alpha * grad[:, 16:22]).clip(min_bounds[8:], max_bounds[8:])
...
def solve_and_collect(pos_bound, num_points):
    ...
    params_u = jax.random.uniform(key5, (1, 6), minval=-1, maxval=1)
    params_up = jnp.array([[EPSILON, 1-EPSILON]])
    params_d = jax.random.uniform(key6, (1, 6), minval=-1, maxval=1)
