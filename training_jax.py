...
def create_train_step(key, model, optimizer, batch_size=128):
    params = model.init(key, jnp.zeros((batch_size, ModelConfig.in_features)))
...
def train(model, optimizer, num_epoch, train_dataloader, model_dir, key):
    ...
    with tqdm(total=len(train_dataloader) * num_epoch) as pbar:
        for epoch in range(num_epoch):
            for i, (batch, gt) in enumerate(train_dataloader):
                ...
                batch = batch.reshape(len(batch), ModelConfig.in_features)
