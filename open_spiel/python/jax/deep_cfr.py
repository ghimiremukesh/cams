...
class DeepCFRSolver(policy.Policy):
  ...
  def solve(self, checkpoints_dir, save_itr):
    """Solution logic for Deep CFR."""
    advantage_losses = collections.defaultdict(list)
    print("Learning advantage Network")
    for _ in tqdm(range(self._num_iterations)):
      ...
      if self._iteration % save_itr == 0 and self._iteration != (self._num_iterations):
        print("Learning policy network at iteration ", self._iteration)
        self._reinitialize_policy_network()
        policy_loss = self._learn_strategy_network()
        save_dir = os.path.join(checkpoints_dir, f'itr_{self._iteration}/')
        save_dir = os.path.abspath(save_dir)
        checkpoints.save_checkpoint(ckpt_dir=save_dir, target=self._params_policy_network, step=self._iteration)
      self._iteration += 1
    # Train policy network.
    print("Learning final policy network")
    self._reinitialize_policy_network()
    save_dir = os.path.join(checkpoints_dir, f'itr_final/')
    save_dir = os.path.abspath(save_dir)
    checkpoints.save_checkpoint(ckpt_dir=save_dir, target=self._params_policy_network, step=self._num_iterations)
    policy_loss = self._learn_strategy_network()
  ...
  def _learn_strategy_network(self):
    ...
    Returns:
      ...
      or `None`.
    """
    with tqdm(total=self._policy_network_train_steps) as pbar:
