# dsgda_solver_jax.py
# =========================================================
# JAX DSGDA solver optimized for multi-GPU training.
# - On-device gradient accumulation using jax.lax.fori_loop to minimize
#   CPU-GPU communication overhead.
# - Data parallelism via jax.pmap.
# =========================================================
...
def _global_grad_norm(grads) -> float:
    sq_norms = [jnp.sum(jnp.array(g, dtype=F32) ** 2) for g in jax.tree.leaves(grads)]
    return (jnp.sqrt(jnp.sum(jnp.array(sq_norms, float))))
...
# =========================================================
# Loss function for a PER-DEVICE micro-batch (Unchanged)
# =========================================================
def make_loss_fn(
    game: FootballGame,
...
class DSGDASolver:
    def __init__(
        ...
        *,
        ...
        assert self.global_batch_size % self.num_devices == 0, \
            "Global batch size must be divisible by number of devices"
        self.per_device_batch_size = self.global_batch_size // self.num_devices
        assert self.per_device_batch_size % self.per_device_micro_batch_size == 0, \
            "Per-device batch size must be divisible by per-device micro batch size"
        ...
        self._pmapped_step = self._create_pmapped_step()

        self.p1_params = flax_utils.replicate(p1_params)
        self.p2_params = flax_utils.replicate(p2_params)

        c1, c2 = float(self.spec.C2_p1) ** 0.5, float(self.spec.C2_p2) ** 0.5
        self.opt_p1 = optax.chain(optax.clip_by_global_norm(c1), optax.sgd(self.spec.lr_p1, self.spec.momentum))
        self.opt_p2 = optax.chain(optax.clip_by_global_norm(c2), optax.sgd(self.spec.lr_p2, self.spec.momentum))
        
        # p1_params is unreplicated here for init, which is correct
    ...
    def _create_pmapped_step(self):
        """Creates the core training step function, including on-device
        gradient accumulation, and wraps it with jax.pmap."""
        ...
        # This is the function that will be executed in parallel on each device.
        def train_step(p1_params, p2_params, opt_state_p1, opt_state_p2, key):

            if key.ndim > 1:
                key = jnp.squeeze(key, axis=0)

            # Initial state for the on-device accumulation loop
            init_grads_p1 = jax.tree.map(jnp.zeros_like, p1_params)
            init_grads_p2 = jax.tree.map(jnp.zeros_like, p2_params)
            init_val = (init_grads_p1, init_grads_p2, 0.0, key)

            # This is the body of the accumulation loop.
            def accum_step_fn(i, val):
                grads_acc_p1, grads_acc_p2, total_loss, loop_key = val
                loop_key, micro_batch_key = jax.random.split(loop_key)
                (loss_micro, _), (gr_p1_micro, gr_p2_micro) = jax.value_and_grad(
                ...
                return (grads_acc_p1, grads_acc_p2, total_loss, loop_key)

            # Run the accumulation loop entirely on the device.
            final_grads_p1, final_grads_p2, total_loss, _ = jax.lax.fori_loop(
            ...
            # Synchronize gradients and loss across all devices.
            ...
            # Also return metrics for logging
            g1_norm = _global_grad_norm(final_grads_p1)
            g2_norm = _global_grad_norm(final_grads_p2)

            return new_p1_params, new_p2_params, new_opt_state_p1, new_opt_state_p2, final_loss, g1_norm, g2_norm
    ...
    def step(self) -> Dict[str, float]:
        ...
        # A single call executes the entire batch, including accumulation.
        (self.p1_params, self.p2_params, 
         self.opt_state_p1, self.opt_state_p2, 
         loss, g1, g2) = self._pmapped_step(
        ...
        rec = {"iter": len(self.meta), "L": loss_host, "g_p1": g1_host, "g_p2": g2_host,
    ...
    def save_checkpoint(self, tag: str | int):
        ...
        meta = {"iter": state_tree["iter"], "stamp": self.stamp, "spec": _to_jsonable(self.spec),
                "game_spec": _to_jsonable(self.game.spec), "S_paths": int(self.S_paths),
