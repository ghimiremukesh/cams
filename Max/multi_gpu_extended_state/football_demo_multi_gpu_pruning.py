# football_demo_pruning.py
# =========================================================
# Driver for JAX-based training, configured for multi-GPU
# and using the solver with tree pruning enabled.
# =========================================================
...
from game_jax import FootballGame, default_football_spec
from player_jax import CAMSInformed, BR
# Import the new solver with pruning capabilities
from solver_jax_multi_gpu_pruning import DSGDASolver
...
PRUNING_SPEC = {
    ...
    "prune_warmup": 1000,
    "eps_fraction": 1e-4,
    "size_ratio_gate": 0.95,
}

EPOCHS      = 50_000
VIS_EVERY   = 2500
LOG_ROOT    = "pruning_test/"
HORIZON     = 2.5
DT          = 0.25
N_SUBSTEPS  = 4
N_PLAYERS   = 11
...
def main():
    ...
    spec = default_football_spec(N=N_PLAYERS, horizon=HORIZON, dt=DT, n_substeps=N_SUBSTEPS)
    ...
    print(f"[cfg] Pruning will be checked every {solver.pruning_spec.prune_every} steps after {solver.pruning_spec.prune_warmup} warmup steps.")

    for epoch in tqdm(range(EPOCHS)):
        stats = solver.step()
        
        if (epoch > 0 and epoch % PRINT_EVERY == 0):
            print(f"\n[{epoch:05d}] L={stats['L']:+.4f} ||g_p1||={stats['g_p1']:.4f} ||g_p2||={stats['g_p2']:.4f} S_paths={stats['S_paths']} wall={stats['wall_ms']:.1f}ms")
        
        if (epoch > 0 and epoch % VIS_EVERY == 0) or epoch == EPOCHS-1:
            ckpt_path = solver.save_checkpoint(epoch)
            print(f"[ckpt] saved {ckpt_path}")
            # visualize_most_likely(...)
