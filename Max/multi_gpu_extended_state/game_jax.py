# game_jax_random_start.py
# =========================================================
# JAX environment for the 2-team American-football running-play game.
# This version initializes the game with random noise applied to the
# standard lineup positions and velocities.
...
def default_football_spec(
    ...
    *,
    ...
    n_substeps: int = 4,
    lineup_pos_std=0.05,   # stdev for position noise (meters)
    lineup_vel_std=0.10,   # stdev for velocity noise (m/s)
) -> Dict[str, Any]:
    twoN = 2 * N
    ...
    spec: Dict[str, Any] = dict(
        ...
        LINEUP_POS_STD=lineup_pos_std,   # stdev for position noise (meters)
        LINEUP_VEL_STD=lineup_vel_std,   # stdev for velocity noise (m/s)
...
class FootballState:
    ...
    ## new addition for multi-gpu
    def tree_replace(self, **kwargs):
        # This manual constructor is the correct and robust way to create a
        # new instance with updated fields for a JAX PyTree. It replaces
        # a previously incorrect implementation.
        return FootballState(
            x=kwargs.get('x', self.x),
            t=kwargs.get('t', self.t),
            p=kwargs.get('p', self.p),
            i_star=kwargs.get('i_star', self.i_star),
            w_last=kwargs.get('w_last', self.w_last),
        )
...
class FootballGame:
    def __init__(self, spec: Dict[str, Any], batch_size: int = 1):
        ...
        # Lineup randomization parameters
        self.pos_std = float(spec.get("LINEUP_POS_STD", 0.0))
        self.vel_std = float(spec.get("LINEUP_VEL_STD", 0.0))
    ...
    def reset(self, key: jax.random.KeyArray = DEFAULT_PRNG_KEY, batch_size: Optional[int] = None) -> Tuple[FootballState, jax.random.KeyArray]:
        """
        Resets the environment to a new initial state for a batch.
        The initial state is based on a standard lineup, but with optional
        Gaussian noise added to player positions and velocities for variety.
        """
        ...
        # 1. Get base deterministic lineup and tile for the batch
        pos1_row, pos2_row = self._lineup()
        pos1_base = jnp.tile(pos1_row[None, :, :], (B, 1, 1))
        pos2_base = jnp.tile(pos2_row[None, :, :], (B, 1, 1))
        vel1_base = jnp.zeros_like(pos1_base)
        vel2_base = jnp.zeros_like(pos2_base)
        
        # 2. Add random noise if specified
        key, p1k, p2k, v1k, v2k, isk = jax.random.split(key, 6)
        
        pos1 = pos1_base + self.pos_std * jax.random.normal(p1k, shape=pos1_base.shape)
        pos2 = pos2_base + self.pos_std * jax.random.normal(p2k, shape=pos2_base.shape)
        vel1 = vel1_base + self.vel_std * jax.random.normal(v1k, shape=vel1_base.shape)
        vel2 = vel2_base + self.vel_std * jax.random.normal(v2k, shape=vel2_base.shape)
        
        # 3. Clip to stay within bounds (important after adding noise)
        pos1 = jnp.clip(pos1, -self.BOX_POS, self.BOX_POS)
        pos2 = jnp.clip(pos2, -self.BOX_POS, self.BOX_POS)
        vel1 = jnp.clip(vel1, -self.BOX_VEL, self.BOX_VEL)
        vel2 = jnp.clip(vel2, -self.BOX_VEL, self.BOX_VEL)

        # 4. Assemble the state vector
        x = jnp.concatenate(
            [
                pos1.reshape(B, -1),
                vel1.reshape(B, -1),
        ...
        i_star = jax.random.randint(isk, (B,), minval=0, maxval=self.I, dtype=jnp.int32)

        w_last = jnp.zeros((B, self.N, self.N), dtype=F32)
        state = FootballState(x=x, t=t, p=p, i_star=i_star, w_last=w_last)
        
        # Return the new state and the *next* PRNG key for subsequent use
        return state, key
    ...
    def _tackle_prob(self, w_merge: jnp.ndarray, pos1: jnp.ndarray, pos2: jnp.ndarray,
        ...
        d = jnp.sqrt(jnp.sum(diff * diff, axis=-1) + 1e-30)          # (B,N) # avoid NaN grad at 0; use low epsilon
