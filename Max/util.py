...
def plot_run(log_path: str, save_png: str | None = None):
    ...
    # 4. stacked per-phase times
    phases = ["t_prune", "t_loss", "t_backward", "t_momentum", "t_step"]
    bottom = None
    for ph in phases:
        ax[3].bar(df["iter"], df[ph], bottom=bottom, label=ph.replace("t_",""))
        bottom = df[ph] if bottom is None else bottom + df[ph]
    ax[3].legend(); ax[3].set_title("Per-iteration CPU time (ms)")
