...
def create_train_step(key, model, config, optimizer, batch_size, params=None):
    if params is None:
        params = model.init(key, jnp.zeros((batch_size, config.in_features)))
        params = initial_cvx(params)
...
def train(model, model_params, config, optimizer, num_epoch, train_dataloader, key):
