...
from dsgda_solver_dual import dsgda_solver, dsgda_solver_final, coarse_solvers_adaptive
...
def sample_states(t, num_points):
    ...
    p_hat = np.random.uniform(-1.4, 1.4, size=((num_points, 2)))
...
def get_smoothing_data(coords, policies, val_model, model_params, t, tau, fine_iters):
    ...
    X_unnorm = util_funcs.unnormalize_states_dual(X, vel_bound, vel_bound, vel_bound, vel_bound)
...
class MultigridFAS:
    def __init__(self, val_model, optim, h=0.1, H=0.2, fine_steps=10, coarse_steps=5, seed=0, batch_size=10000,
                 fine_iters=10000):
    ...
    def cycle(self):
        ...
        for t in range(self.fine_steps):
            ...
            X_unnorm = util_funcs.unnormalize_states_dual(self.X[t], vel_bound, vel_bound, vel_bound, vel_bound)
            if t == self.fine_steps - 1:
                target, policy = dsgda_solver_final(X_unnorm, self.h, iters=self.fine_iters)
            else:
        ...
        for t in reversed(range(self.coarse_steps)):
            if t == self.coarse_steps - 1:
                ...
                correction = coarse_solvers_adaptive(augmented_val_fn, v_Hs[t+1], self.X[t], t, self.H, self.h)
                correction -= coarse_residuals[t]
        ...
        # TODO: Prolong coarse correction to fine correction
        for t in range(0, self.coarse_steps):
            self.fine_corr[2*t] = self.coarse_corr[t]
            if 2*t-1 > 0:
                self.fine_corr[2*t-1] = self.coarse_corr[t]
        # for t in range(0, self.fine_steps-1, 2):
        #     self.fine_corr[t] = self.coarse_corr[t]
        #     if t+1 != self.fine_steps - 1:
        #         self.fine_corr[t+1] = self.coarse_corr[t+1]

        # Fit the fine value networks to the correction. No correction in the last one
