...
# Lint as python3
"""Hexner's one-step game w/ nn value function implemented in Python.
...
class HexnerGame(pyspiel.Game):
    ...
    def __init__(self,
                 ...
                 value_fn: Optional = None):
        self.payoff = value_fn
        # self.p = params['belief']
        self.t = params['time-step']
    ...
    def new_initial_state(self):
        ...
        return HexnerState(self, self.initial_state, self.p, self.t, self.value_fn)
...
class HexnerState(pyspiel.State):
    ...
    def __init__(self, game, initial_state, p, t, val_fun, ):
    ...
    def _apply_actions(self, actions):
        ...
        _next_states, _ins_costs = self._go_forward(self.states[-1], actions)
    ...
    def _terminal_cost(self, states):
        ...
        if self.time >= 0.9:
            ...
            terminal = dist_to_goal_1 * self.p + (1 - self.p) * dist_to_goal_2
        else:
            coords = np.concatenate((states, np.array(self.p)))
            coords = torch.from_numpy(coords).to(torch.float32)
            coords = {'coords': coords}
            terminal = self.val_fun(coords)['model_out'][0].item()
    ...
    def _action_to_string(self, player, action):
        ...
        if player == pyspiel.PlayerId.CHANCE:
            return f"Type:{'L' if action == 0 else 'R'}"
        if player == 0:
    ...
    def returns(self):
        ...
        return np.round(self._returns, 2)  # round off for better graph viz.
...
class HexnerObserver:
    ...
    def __init__(self, iig_obs_type, params):
        ...
        # if iig_obs_type.private_info == pyspiel.PrivateInfoType.SINGLE_PLAYER:  # private info about type
        #     pieces.append(("player_type", 2, (2, )))
        #
