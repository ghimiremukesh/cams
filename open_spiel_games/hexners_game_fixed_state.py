...
_NUM_PLAYERS = 2
_TYPE = frozenset([0, 1])
_dt = 1 # 0.25 for 4-stages
_GAME_LENGTH = int(1/_dt)  # for obs shape
_R1 = np.array([[0.05, 0], [0, 0.025]])
_R2 = np.array([[0.05, 0], [0, 0.1]])
# _BELIEF = frozenset([0.5, 0.5])
_GAME_TYPE = pyspiel.GameType(
    short_name="python_hexner_fixed_state",
...
_GAME_INFO = pyspiel.GameInfo(
    ...
    max_utility=1e6,
    utility_sum=0.0,
    max_game_length=_GAME_LENGTH)  # e.g. total time-steps
    ...
    def __init__(self, iig_obs_type, params):
        ...
        if iig_obs_type.public_info:
            if iig_obs_type.perfect_recall:
                ...
                # pieces.append(("actions", 2 * _GAME_LENGTH * n**2, (_GAME_LENGTH, 2 * n**2))) # remove for single stage
