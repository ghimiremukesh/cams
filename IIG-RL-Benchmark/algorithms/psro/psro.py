...
from algorithms import abstract_meta_trainer
...
class PSROSolver(abstract_meta_trainer.AbstractMetaTrainer):
    ...
    def update_agents(self):
        ...
        (sample_strategy, total_policies, probabilities_of_playing_policies) = (
            self.get_policies_and_strategies()
        )
        ...
        self._new_policies = self._oracle(
            ...
            using_joint_strategies=self._rectify_training
            or not self.sample_from_marginals,
        )
    ...
    def update_empirical_gamestate(self, seed=None):
        ...
        updated_policies = [
            self._policies[k] + self._new_policies[k] for k in range(self._num_players)
        ]
            ...
            for current_index in itertools.product(*range_iterators):
                ...
                if np.isnan(meta_games[current_player][tuple(used_index)]):
                    estimated_policies = (
                        ...
                        + [
                            self._new_policies[current_player][
                    ...
                    if self.symmetric_game:
                        ...
                        utility_estimates = self.sample_episodes(
                            estimated_policies, self._sims_per_entry
                        )

                        player_permutations = list(
                            itertools.permutations(list(range(self._num_players)))
                        )
    ...
    def get_num_meta_game_steps(self):
        """Returns number of steps taken in training the meta game."""
        return self._num_meta_game_steps

    def get_num_oracle_steps(self):
        return self._oracle._num_steps
