...
import pyspiel
from algorithms import fine_tuning
import enum
...
def optax_optimizer(
    params: chex.ArrayTree, init_and_update: optax.GradientTransformation
) -> Optimizer:
...
class RNaDSolver(policy_lib.Policy):
    ...
    def __init__(self, config, game):
        ...
        self.actor_steps = 0
        self._num_steps = 0
        self.finetune = fine_tuning.FineTuning()
    ...
    def init(self, game):
        ...
        self._game = game  # pyspiel.load_game(self.config.game)
        ...
        self._entropy_schedule = EntropySchedule(
            sizes=tuple([self.config.entropy_schedule_size_value,]),
            repeats=tuple(self.config.entropy_schedule_repeats),
        )
    ...
    def loss(
        ...
        policy_pprocessed = self.finetune(pi, ts.env.legal, learner_steps)
        ...
        loss_v = get_loss_v(
            [v] * self._game.num_players(), v_target_list, has_played_list
        )
    ...
    def update_parameters(
        ...
        loss_val, grad = self._loss_and_grad(
            params,
            params_target,
            params_prev,
            params_prev_,
    ...
    def _network_jit_apply_and_post_process(
        self, params: Params, env_step: EnvStep
    ) -> chex.Array:
    ...
    def _batch_of_states_apply_action(
        self, states: Sequence[pyspiel.State], actions: chex.Array
    ) -> Sequence[pyspiel.State]:
